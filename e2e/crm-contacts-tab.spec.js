...
/*
 * The Contacts-app integration injects a "CRM Notes" panel into the contact
...
test.describe('CRM Notes — Contacts app integration', () => {
	test('the byContact API (used by the injected panel) returns a contact\'s notes', async ({ page }) => {
		...
		const result = await page.evaluate(async (title) => {
			...
			const create = await fetch('/index.php/apps/crm_notes/api/notes', {
			...
			const byContact = await fetch('/index.php/apps/crm_notes/api/notes/contact/' + encodeURIComponent(uid), { headers: { requesttoken: token } });
			const list = await byContact.json();
			await fetch('/index.php/apps/crm_notes/api/notes/' + note.id, { method: 'DELETE', headers: { requesttoken: token } });
	...
	test('the standalone app honours the panel\'s #contact/UID deep link', async ({ page }) => {
		// The Contacts panel\'s "open in CRM Notes" link points at
		// /apps/crm_notes#contact/<uid>; loading that route must open the app
		...
		await page.goto('/apps/crm_notes/#contact/' + encodeURIComponent(uid));
	...
	test('the injected panel renders when the Contacts app exposes the contact detail', async ({ page }) => {
		...
		if (!appeared) {
			test.info().annotations.push({ type: 'note', description: 'CRM Notes panel was not injected into the Contacts detail view (LoadContactsOcaApiEvent not observed firing). Integration contract is covered by the byContact-API + deep-link tests.' });
		...
		await expect(openLink).toHaveAttribute('aria-label', /CRM Notes|neuen Tab|new tab/i);
		await expect(openLink).toHaveAttribute('href', /crm_notes.*#contact\//);
	});
});
